...
All three entries below were fixed together via the `spec-stage-tracking-fixes` wayfinder trail (2026-07-20).

---
## `rig` MCP tools never expose a task item's real UUID to a fresh reader

**Fixed (2026-07-20, commit `f3f915d`):** Added `list_task_items(specId, componentSlug)` and `list_definition_of_done_items(specId)` MCP read tools in `packages/server/src/mcp/tools/tasks-tools.ts`, wrapping pre-existing `SpecRepository` methods that already returned the right shape. Both return each item's real `id` alongside its display `itemId`/ordinal, title/description, `isChecked`, and `parentItemId` — verified live against the `workspace-based-project-binding` spec (`list_task_items` on the `workspace-scanner` component returns real UUIDs like `066fa7ba-879c-412f-83f5-be1a138ed65b` for display id `"1"`) and by round-trip tests in `tasks-tools.test.mjs` that take a listed id straight into `update_task_item`/`update_definition_of_done_item` and confirm it succeeds. `render_document`'s markdown output was deliberately left unchanged (read-tool-only fix, per the trail's locked decision).
...
**Fixed (2026-07-20, commit `992e8cd`):** `currentStage` is now derived at read time in `spec-repository.ts` via a shared `deriveCurrentStage` helper (built alongside `deriveTasksAggregateStatus`, factored out of `get_next_stage`'s existing correct logic) rather than read off the stored `specs.current_stage` column. `getSpec`, `getSpecBySlug`, and `listSpecs` all use it now. The `specs.current_stage` column itself was subsequently dropped from the live schema (see the entry below) — verified live on the `workspace-based-project-binding` spec: `get_spec` now correctly returns `currentStage: "tasks"` (previously stuck at `"requirements"`).
...
**Fixed (2026-07-20, commits `992e8cd` + `b5972ed`):** `get_spec`'s `stages` array now derives the `tasks` entry's status live via `deriveTasksAggregateStatus` (all `tasks_docs` rows approved → `approved`, matching `get_next_stage`'s existing correct check) instead of reading the stuck `spec_stages` row. The now-fully-vestigial `spec_stages` row for `stage_name = 'tasks'` was then retired outright: `seed_spec_stages()` no longer inserts it, the column comment in `spec-templates/spec/db/schema.sql` documents the new reality, and the migration was applied against the live `rig-postgres-1` container (`spec_pipeline.spec_stages` now only ever holds `requirements`/`design` rows, confirmed live: `SELECT stage_name, count(*) ... GROUP BY stage_name` returns only those two). Since nothing physically exists to override anymore, `getSpecStages` and `reopen_trail`'s `specStatus` reporting now synthesize a full tasks-stage entry from the derived helper. Verified live on `workspace-based-project-binding`: `stages` reports `tasks: approved`, matching all 5 approved `tasksDocs` components and `get_next_stage`'s `actionableStage: null`.
