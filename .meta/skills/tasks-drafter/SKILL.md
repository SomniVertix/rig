...
Everything that leaves your head stays plain, uncompressed English: the tasks documents,
...
available right now: run `ls ~/.claude/skills/*/SKILL.md` (implementation specialists like
`code-implementer`/`test-writer`/`code-reviewer` are Claude Code skills, dispatched by the
orchestrator via the `Skill` tool, not `.claude/agents/` subagents) plus the built-in agent
types (`general-purpose`, `Explore`, `Plan`, etc.). Assign the best-suited one per task by
name. If none fits, use `none` — the implementer falls back to its default agent for that
task. Do not assume a fixed roster; re-check every time, since available agents can change
between specs.
...
belongs in that batch. Items in the same batch must be safe to run concurrently. Note: the
orchestrator's actual dispatch of a batch runs each item's skill sequentially, not with true
wall-clock concurrency (`Skill(context: fork)` dispatch doesn't parallelize the way the old
`Agent`-tool dispatch did) — batches are still meaningful as a correctness/ordering grouping,
just not a speed-up.
...
your best-effort task list anyway and record the concern via `add_tasks_flag` — you return
one final message and cannot pause for a human Q&A turn.
