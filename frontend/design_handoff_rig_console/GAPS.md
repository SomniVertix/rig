...
SSE pings. V2 (`backend/cmd/rig`, `backend/internal/graph`, `backend/internal/binding`) is a
...
(`backend/cmd/rig/main.go`) on port 8789 (`RIG_HTTP_ADDR`), with no separate BFF layer and
...
list as a stopgap — see `frontend/src/config/projects.ts` once it exists — pending a
