...
export const REGISTERED_FILTERS = ['json', 'default', 'truncate', 'fromjson'] as const;
...
function parsePath(expression: string): Array<string | number> {
...
function resolveReference(expression: string, scope: TemplateContext): unknown {
				...
				: root === 'prev'
					...
					: root === 'nodes'
						...
						: root === 'node'
							...
							: root === 'inputs'
								? scope.inputs
								: (scope.context as Record<string, unknown>)[String(root)];
		return rest.length === 0 ? base : rest.reduce<unknown>((current, segment) => {
...
function evaluateSimpleCondition(expression: string, scope: TemplateContext): boolean {
	const trimmed = expression.trim();
	if (trimmed === '') {
		return false;
	}
...
export function evaluateGuardExpression(expression: string, context: TemplateContext): boolean {
