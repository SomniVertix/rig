...
async function testWithEnv(envOverrides, configOverrides = {}) {
	...
	const source = {
		...
		configPath: '/tmp/test/rig.config.ts'
	};
...
describe('loadServerConfig webPort/webHost configuration', () => {
	describe('webPort field', () => {
		test('(a) RIG_WEB_PORT unset -> webPort undefined', async () => {
			const config = await testWithEnv({
				// Explicitly not setting RIG_WEB_PORT
			});
			assert.equal(config.webPort, undefined, 'webPort should be undefined when RIG_WEB_PORT is unset');
		});

		test('(b) valid RIG_WEB_PORT -> numeric webPort', async () => {
			const config = await testWithEnv({
				RIG_WEB_PORT: '3000'
			});
		...
		test('(b) valid RIG_WEB_PORT with large number', async () => {
			const config = await testWithEnv({
				RIG_WEB_PORT: '65535'
			});
		...
		test('(b) valid RIG_WEB_PORT with minimum value 1', async () => {
			const config = await testWithEnv({
				RIG_WEB_PORT: '1'
			});
		...
		test('(c) invalid RIG_WEB_PORT (zero) -> ConfigError', async () => {
		...
		test('(c) invalid RIG_WEB_PORT (negative) -> ConfigError', async () => {
			await assert.rejects(
				async () => testWithEnv({
					RIG_WEB_PORT: '-100'
				}),
		...
		test('(c) invalid RIG_WEB_PORT (non-integer) -> ConfigError', async () => {
			await assert.rejects(
				async () => testWithEnv({
					RIG_WEB_PORT: '3000.5'
				}),
		...
		test('(c) invalid RIG_WEB_PORT (non-numeric) -> ConfigError', async () => {
			await assert.rejects(
				async () => testWithEnv({
					RIG_WEB_PORT: 'not-a-number'
				}),
				(error) => error instanceof ConfigError && error.key === 'webPort',
		...
		test('(c) invalid RIG_WEB_PORT (empty string) -> webPort undefined', async () => {
			const config = await testWithEnv({
				RIG_WEB_PORT: ''
			});
		...
		test('(c) invalid RIG_WEB_PORT (whitespace only) -> webPort undefined', async () => {
			const config = await testWithEnv({
				RIG_WEB_PORT: '   \t  '
			});
	...
	describe('webHost field', () => {
		test('(d) webHost defaults to 0.0.0.0 when unset', async () => {
			const config = await testWithEnv({
				// Explicitly not setting RIG_WEB_HOST
			});
			assert.equal(config.webHost, '0.0.0.0', 'webHost should default to 0.0.0.0 when RIG_WEB_HOST is unset');
		});

		test('webHost accepts custom value from RIG_WEB_HOST', async () => {
			const config = await testWithEnv({
				RIG_WEB_HOST: 'localhost'
			});
			assert.equal(config.webHost, 'localhost', 'webHost should use the RIG_WEB_HOST value when set');
		});

		test('webHost accepts 127.0.0.1', async () => {
			const config = await testWithEnv({
				RIG_WEB_HOST: '127.0.0.1'
			});
