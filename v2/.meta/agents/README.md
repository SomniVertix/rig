...
they are not live and the rig server never reads this directory directly. To deploy
one, copy it into `~/.claude/skills/<name>/SKILL.md`, converting frontmatter per "Deploying
an agent" below. The rig server's `known_actors` registry (Story 11.5's boot sync)
...
`pre/decisions.md` holds the grilling transcript (2026-07-13) that decided this set of
seven agents in the first place — which stages get a dedicated agent, tool access per agent,
and model choice per agent, each with rationale. History only: not read by any agent or the
rig server, kept here so the reasoning behind the roster below stays reviewable.
...
All spec data is managed entirely through the `rig` MCP server
(`mcp__rig__*` tools), never the local filesystem. There is no `.rig/specs/`
...
by calling `mcp__rig__get_spec` before acting — there is no separate
router/dispatcher agent.

| Agent | Stage | Reads | Writes | Model | Why this model |
|---|---|---|---|---|---|
| `requirements-compiler` | 1 | the spec's linked expedition via `mcp__rig__get_expedition_by_spec` — reached waypoints as the decisions transcript, bypassed waypoints as out-of-scope rulings, expedition terms as glossary (nothing is handed in-context) | requirements, via `mcp__rig__*` | Sonnet | Reliable structured reformatting (EARS notation) on a foundational document; Opus is overkill, Haiku risks sloppy output. |
| `design-drafter` | 2 | requirements (`render_document`) + codebase | design, via `mcp__rig__*` | Opus | Highest-stakes stage — real architectural decisions and alternatives, no human in the loop until approve/deny. Worth the cost. |
| `tasks-drafter` | 3 | design (`render_document`), plus requirements for traceability targets | tasks, via `mcp__rig__*` | Sonnet | Correctness-critical linear decomposition and execution ordering; more careful decomposition than creative judgment. |
...
Each drafting agent (`requirements-compiler`, `design-drafter`, `tasks-drafter`) can't hold a
back-and-forth with the human mid-task — subagents invoked via the Agent tool return a
single final message. When it hits a real gap, it raises an `add_open_question` (one
unified artifact type across all three stages, replacing v1's separate assumptions/design
flags/tasks flags) and ends its turn *without* calling `finalize_stage` for the affected
part — an unresolved open question blocks both `finalize_stage` and approve, so there's no
way to silently draft around it. A human answers it via `resolve_open_question`, then a
fresh invocation of the same agent reads everything cold again, reconciles, and finishes.
...
`mcp__rig__get_spec`/stage-approval preconditions in its own body before acting, so
...
Agents and Claude Code skills are separate concepts — the rig server's boot sync
(`syncKnownActorsFromActorsDirectory`, `packages/server/src/mcp/guardrails/actor-registry.ts`)
scans a curated `rig-actors` directory, not the general skills directory, so that
...
```
ln -s ../skills/<name> ~/.claude/rig-actors/<name>
```

This applies to every one of the seven agents below, plus the discovery-stage skills that
write directly to rig (`grilling`, and `wayfinder` if present) even though those
